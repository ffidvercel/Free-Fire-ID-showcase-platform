    ...
    <Card className="flex flex-col overflow-hidden h-full transition-all duration-300 hover:shadow-lg hover:-translate-y-1 focus-within:shadow-lg focus-within:-translate-y-1">
        ...
        <CardHeader className="p-0">
          <div className="relative aspect-[4/3] w-full">
            ...
            <div className="absolute bottom-2 left-2">
              <Badge className="bg-green-500/90 hover:bg-green-500 text-[10px] py-0 px-1.5 h-5 flex items-center gap-1 border-none shadow-sm">
      ...
      <CardFooter className="p-4 pt-2 flex-col items-start gap-3">
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-2">
          <Button asChild variant="outline" size="sm" className="w-full text-accent group border-accent/30 hover:bg-accent/5">
            <Link href={`/ids/${gameId.id}`}>
              View Details
              <ArrowRight className="ml-1.5 h-3.5 w-3.5 transition-transform group-hover:translate-x-1" />
            </Link>
          </Button>
          <Button asChild size="sm" className="w-full bg-green-500 hover:bg-green-600 text-white shadow-sm">
